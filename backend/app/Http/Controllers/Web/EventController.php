...
class EventController extends Controller
{
    ...
    public function update(EventUpdateRequest $request, Event $event)
    {
        ...
        if (! $request->boolean('autosave') && $hasExplicitStaffInput) {
            $teamIds = TeamAssignment::resolveUserIds($meta['equipa_de_trabalho'] ?? null);
    ...
    public function destroy(Event $event)
    {
        ...
        Audit::log('event.cancelled', Event::class, $event->id);

        return redirect()->route('events.index')->with('ok', 'Evento cancelado');
    }
    ...
    public function staffAssign(Request $request, Event $event)
    {
        ...
        app(\App\Support\EventInviteService::class)->sendForEvent($event, 'updated');
    ...
    public function staffRemove(Event $event, User $user)
    {
        ...
        \App\Models\EventStaff::where('event_id', $event->id)
            ...
            ->delete();
        $invite = app(\App\Support\EventInviteService::class);
        $invite->sendCancellation($event, $user, $staff);
