@extends('layouts.app')
@section('content')
<h1 class="text-xl font-semibold mb-4">Editar ficha de evento</h1>
<div class="flex items-center gap-3 mb-2">
    <div id="autosave-status" class="text-xs text-gray-500"></div>
    @if(($event->status ?? null) === 'rascunho')
...
@endphp
<form id="event-edit-form" method="post" action="{{ route('events.update', $event) }}" class="bg-white p-4 rounded shadow space-y-6" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="border rounded p-4 space-y-3">
        ...
        <div class="grid md:grid-cols-3 gap-3">
            ...
            <div>
                ...
                <input type="date" name="event_date" id="event-date" value="{{ $event->event_date->format('Y-m-d') }}" class="border p-2 rounded w-full" required>
            </div>
            <div>
                <label class="block text-sm">Nº reportagem</label>
                <input name="legacy_report_number" value="{{ $event->legacy_report_number ?? '' }}" placeholder="Gerado automaticamente" class="border p-2 rounded w-full">
            </div>
        </div>
        <div class="grid md:grid-cols-4 gap-3">
            ...
            <div>
                ...
                <input name="base_price" type="number" step="0.01" value="{{ $event->base_price }}" class="border p-2 rounded w-full" required>
            </div>
            <div>
    ...
    <div id="event-meta-casamento" class="hidden border rounded p-4 space-y-4">
    ...
    <div id="event-meta-batizado" class="hidden border rounded p-4 space-y-4">
    ...
    <div class="border rounded p-4 space-y-3">
        ...
        <div class="grid md:grid-cols-2 gap-3">
            ...
            <div>
                ...
                <input name="event_meta[igreja_localidade]" value="{{ $meta['igreja_localidade'] ?? '' }}" class="border p-2 rounded w-full">
            </div>
            <div>
            ...
            <div>
                ...
                <input name="event_meta[almoco_localidade]" value="{{ $meta['almoco_localidade'] ?? '' }}" class="border p-2 rounded w-full">
            </div>
        </div>
    </div>
    ...
    <div class="border rounded p-4 space-y-4">
        ...
        <div class="grid md:grid-cols-2 gap-3">
            <div>
                ...
                <input type="number" min="0" name="event_meta[servico_num_profissionais]" value="{{ $meta['servico_num_profissionais'] ?? '' }}" class="border p-2 rounded w-full">
            </div>
            <div>
                <label class="block text-sm">Prazo de entrega</label>
                <input name="event_meta[servico_prazo_entrega]" value="{{ $meta['servico_prazo_entrega'] ?? '' }}" class="border p-2 rounded w-full">
            </div>
            <div>
    ...
    <div class="border rounded p-4 space-y-3">
        ...
        <div class="grid md:grid-cols-2 gap-3">
            <div>
                <label class="block text-sm">Atualizar foto (se tiver câmara)</label>
                <input type="file" name="event_meta[foto_noivos]" accept="image/*" capture="environment" class="border p-2 rounded w-full bg-white">
            </div>
            <div>
                @if(!empty($meta['foto_noivos']))
                    <div class="text-sm text-gray-500 mb-1">Pré-visualização</div>
                    <img src="{{ asset('storage/'.$meta['foto_noivos']) }}" alt="Foto dos noivos" class="max-h-40 rounded border">
                @else
                    <div class="text-sm text-gray-500">Sem foto guardada.</div>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="border rounded p-4 space-y-3">
        ...
        <div class="grid md:grid-cols-2 gap-3">
            <div>
                ...
                <input type="time" name="event_meta[estar_na_loja_as]" value="{{ $meta['estar_na_loja_as'] ?? '' }}" class="border p-2 rounded w-full">
            </div>
        </div>
        <div>
            <label class="block text-sm">OBS</label>
            <textarea name="notes" class="border p-2 rounded w-full" rows="3">{{ $event->notes }}</textarea>
        </div>
    </div>
