@extends('layouts.app')
@section('content')
<h1 class="text-xl font-semibold mb-4">Nova ficha de evento</h1>
...
@php
    $meta = old('event_meta', []);
@endphp
<form id="event-create-form" method="post" action="{{ route('events.store') }}" class="bg-white p-4 rounded shadow space-y-6" enctype="multipart/form-data" data-update-url-template="{{ route('events.update', ['event' => '__EVENT__']) }}">
    @csrf
