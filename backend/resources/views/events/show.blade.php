...
<div class="flex items-center justify-between mb-4">
    <div>
        <h1 class="text-xl font-semibold">Dossier: {{ $event->name }}</h1>
        <p class="text-sm text-gray-600">{{ $event->event_date->format('Y-m-d') }}</p>
    </div>
...
<div class="bg-white border rounded p-3 mb-2">
    ...
    @if(auth()->user()->hasPermission('photos.manage'))
